...
web.get('/', function(req, res){
    res.render('index.ejs', {layout: false, servers: config.servers});
});
...
connection.on('ready', function () {
    ...
    q.subscribe(function (msg) {
        ...
        if (typeof stats.memory_total !== 'undefined' && typeof stats.memory_free !== 'undefined') {
            stats.memory = {};
            stats.memory.low = parseInt((stats.memory_total/3)/1024);
            stats.memory.high = stats.memory.low * 2;
            stats.memory.total = parseInt(stats.memory_total/1024);
            stats.memory.used = stats.memory.total - parseInt(stats.memory_free/1024);
